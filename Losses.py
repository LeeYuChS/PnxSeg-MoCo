...
def iou_metric(preds, trues, per_image=False, per_channel=False):
    """
    IoU (Intersection over Union) metric, also known as Jaccard index
    """
    preds = preds.float()
    batch_size, n_channels = preds.size(0), preds.size(1)
    ...
    target = trues.contiguous().view(n_parts, -1).float()
    output = preds.contiguous().view(n_parts, -1)
    intersection = torch.sum(output * target, dim=1)
    union = torch.sum(output, dim=1) + torch.sum(target, dim=1) - intersection + eps
