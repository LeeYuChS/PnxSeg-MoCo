...
# import os
# def create_moco(weights=None, device="cuda"):
#     moco_dim = 128
#     moco_k = 4096
#     moco_m = 0.99
#     moco_t = 0.1
...
#         incompat = moco_model.load_state_dict(state, strict=False)
#         # print(f"Incompatible keys: {incompat}")  # NOW SHOULD BE EMPTY OR MINOR (NO SHAPE MISMATCH)
    
#     # SET RETURN_DICT
#     encoder_q = moco_model.encoder_q
#     # if hasattr(encoder_q, 'model'):
#     #     encoder_q.model.config.use_return_dict = True
#     #     if hasattr(encoder_q.model, 'vision_model'):
#     #         encoder_q.model.vision_model.config.use_return_dict = True
#     # elif hasattr(encoder_q, 'vision_model'):
#     #     encoder_q.vision_model.config.use_return_dict = True
#     # else:
#     #     print("Warning: No inner model found!")
...
# def get_GroupViTSegmenter(
#     device="cuda" if torch.cuda.is_available() else "cpu"
# ):
#     weights = os.path.join("./", "XrayPnxSegment/models", "model_last.pth")
#     query_encoder = create_moco(weights)
#     # print(query_encoder)
#     groupvit_model = GroupViTSegmenter(query_encoder, num_classes=1).to(device)
#     # print(get_trainable_params(groupvit_model))
#     return groupvit_model

# if __name__=="__main__":
#     from transformers import AutoImageProcessor
#     from PIL import Image

#     model = get_GroupViTSegmenter(device="cuda", num_classes=1, freeze_encoder=True)
#     processor = AutoImageProcessor.from_pretrained("nvidia/groupvit-gcc-yfcc")
#     image = Image.open(r"G:\XrayPnxSegment-main\siim-acr-pneumothorax\png_images\0_test_1_.png").convert("RGB")
#     inputs = processor(images=image, return_tensors="pt").to("cuda")

#     model.eval()
#     with torch.no_grad():
#         output = model(inputs['pixel_values'])
#         print(f"Output shape: {output.shape}")  # 應為 [1, 2, 224, 224]

#     # 檢查 vision_outputs 格式
#     vision_outputs, _, _ = model.encoder(inputs['pixel_values'], output_hidden_states=True, output_attentions=False, return_dict=True)
